...
def random_slice(lst, new_len):
    """Randomly select a slice of the list with length new_len"""
    start = random.randint(0, len(lst) - new_len)  # Random start point
    return lst[start:start + new_len]

def truncate_seq_pair_alt(tokens_a, tokens_b, max_num_tokens):
    # Calculate the total length of both sequences
    ...
    # Step 1: Determine how many tokens we need to remove in total
    to_remove = total_length - max_num_tokens

    # Step 2: If one list is significantly longer, remove from the larger list first
    if len(tokens_a) > len(tokens_b):
        excess_a = len(tokens_a) - len(tokens_b)
        if excess_a >= to_remove:
            # We can remove all necessary tokens from tokens_a
            tokens_a = random_slice(tokens_a, len(tokens_a) - to_remove)
            return tokens_a, tokens_b
        else:
            # Remove as much as we can from tokens_a, then adjust to_remove
            tokens_a = random_slice(tokens_a, len(tokens_a) - excess_a)
            to_remove -= excess_a

    elif len(tokens_b) > len(tokens_a):
        excess_b = len(tokens_b) - len(tokens_a)
        if excess_b >= to_remove:
            # We can remove all necessary tokens from tokens_b
            tokens_b = random_slice(tokens_b, len(tokens_b) - to_remove)
            return tokens_a, tokens_b
        else:
            # Remove as much as we can from tokens_b, then adjust to_remove
            tokens_b = random_slice(tokens_b, len(tokens_b) - excess_b)
            to_remove -= excess_b

    # Step 3: If both lists are of equal length, or after trimming the larger list,
    #         remove the remaining tokens equally from both lists.
    if to_remove > 0:
        half_remove = to_remove // 2  # Divide the removal between both lists
        # Remove from tokens_a
        tokens_a = random_slice(tokens_a, len(tokens_a) - half_remove)
        # Remove from tokens_b
        tokens_b = random_slice(tokens_b, len(tokens_b) - (to_remove - half_remove))
    ...
    def worker(self, proc_id, start, end):
        ...
        with open(self.corpus_path, mode="r", encoding="utf-8") as f:
            while pos < start:
                ...
                pos += 1
            if proc_id == 7:
                printc('Started proc 7')
            while True:
                line = f.readline()
                pos += 1
                printc(pos)
                processed_lines += 1
                progress_message = f"Worker {proc_id} processed {processed_lines}/{total_lines} lines"
                if proc_id == 1:
                    if processed_lines >= 112855 and processed_lines <=112858:
                        continue
                ...
                if pos >= end:
                    ...
                    if len(docs_buffer) > 0:
                        printc('debug 2')
                        printc(len(docs_buffer))
                        val = False
                        #if proc_id == 7:
                            #val = True
                        instances = self.build_instances(docs_buffer, val)
                        printc('debug 3')
                        #print(instances)
                        for instance in instances:
                            pickle.dump(instance, dataset_writer)
                        printc('debug 4', proc_id)
                    break
                #if proc_id == 7:
                    #print("debug 1")
                if not line.strip():
        ...
        #with open('progress.txt', 'a') as de:
            #de.write("Worker " + str(proc_id) + " done!!\n")
        print('Worker ' + str(proc_id)+ ' done!!!')
        dataset_writer.close()

    def build_instances(self, all_documents, pr=False):
        instances = []
        for i in range(self.dup_factor):
            for doc_index in range(len(all_documents)):
                #print(i, doc_index)
                instances.extend(self.create_ins_from_doc(all_documents, doc_index, pr))
                #print(instances)
        return instances

    def create_ins_from_doc(self, all_documents, document_index, pr=False):
        ...
        current_length = 0
        i = 0
        printc('document_index', document_index)
        while i < len(document):
            segment = document[i]
            current_chunk.append(segment)
            current_length += len(segment)
            if pr:
                printc(current_length, ": current length")
                printc(len(document), ': len_doc')
                printc(len(all_documents))
            if i == len(document) - 1 or current_length >= target_seq_length:
                if current_chunk:
                    ...
                    if not pr :
                        printc('a_end:', a_end)
                        printc('len(current_chunk):', len(current_chunk))
                    tokens_a = []
                    for j in range(a_end):
                        tokens_a.extend(current_chunk[j])
                    if pr:
                        printc('Done extending tokens_a')
                    tokens_b = []
                    is_random_next = 0
                    ...
                    if len(current_chunk) == 1 or random.random() < 0.5:
                        ...
                        if not pr:
                            printc('length of random doc:', len(random_document))
                            printc('random_start:' , random_start)
                        for j in range(random_start, len(random_document)):
                            if not pr:
                                printc('len of rand:', len(random_document[j]))
                    ...
                    if not pr:
                        printc('before truncate')
                        printc('len of tokens_a,b:', len(tokens_a), len(tokens_b))
                    tokens_a, tokens_b = truncate_seq_pair_alt(tokens_a, tokens_b, max_num_tokens)
                    if not pr:
                        printc(len(tokens_a), len(tokens_b))
                        printc('after truncate')   
                    ...
                    while len(src) != self.seq_length:
                        src.append(PAD_ID)
                    if pr:
                        printc('before masking')   
                    if not self.dynamic_masking:
                        if pr:
                            printc('not in dynamic masking')
