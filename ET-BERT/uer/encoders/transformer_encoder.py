...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, args):
        ...
        if args.target == "tinybert":
            if args.teacher_model:
                self.layers_num = args.teacher_layers_num
    ...
    def forward(self, emb, seg):
        ...
        for i in range(self.layers_num):
            if self.parameter_sharing:
                hidden, attn  = self.transformer(hidden, mask, position_bias=position_bias)
            else:
                hidden, attn = self.transformer[i](hidden, mask, position_bias=position_bias)
            hidden_list.append(hidden)
            attentions.append(attn)

        if self.layernorm_positioning == "pre":
            return self.layer_norm(hidden), attentions, hidden_list, emb1, seg1
        else:
            return hidden, attentions, hidden_list, emb1, seg1
