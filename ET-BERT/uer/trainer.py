...
from uer.model_loader import load_from_teacher
from uer.model_saver import save_model
from uer.model_builder import build_model
from uer.model_builder import build_teacher_model
from uer.model_loader import load_teacher_model
...
def train_and_validate(args):
    ...
    # Build model.
    if args.target == "tinybert":
        teacher_model = build_teacher_model(args)
        teacher_model = load_teacher_model(args, teacher_model)
        teacher_model.eval()
        model = build_model(args, teacher_model)
    else:
        model = build_model(args)

    #if args.teacher_model_path is not None:
    #   model = load_from_teacher(model, args.teacher_model_path)
...
class TinyBertTrainer(Trainer):
    def __init__(self, args):
        super(TinyBertTrainer, self).__init__(args)
        self.total_loss_ce = 0.0
        self.total_loss_att = 0.0
        self.total_loss_hid = 0.0
        self.total_correct = 0.0
        self.total_instances = 0.0
    ...
    def forward_propagation(self, batch, model):
        src, tgt_mlm, tgt_sp, seg = batch
        loss_info = model(src, (tgt_mlm, tgt_sp), seg)
        logits_loss, attention_loss, hidden_loss = loss_info
        loss = logits_loss + attention_loss + hidden_loss
        self.total_loss += loss.item()
        self.total_loss_ce += logits_loss.item()
        self.total_loss_att += attention_loss.item()
        self.total_loss_hid += hidden_loss.item()
        loss = loss / self.accumulation_steps
        return loss
    ...
    def report_and_reset_stats(self):
        ...
        print("| {:8d}/{:8d} steps"
              ...
              "| loss_ce: {:3.3f}"
              "| loss_att: {:3.3f}"
              "| loss_hid: {:3.3f}".format(
            self.current_step,
            self.total_steps,
            done_tokens / (time.time() - self.start_time),
            self.total_loss / self.report_steps,
            self.total_loss_ce / self.report_steps,
            self.total_loss_att / self.report_steps,
            self.total_loss_hid / self.report_steps)) 
        self.total_loss, self.total_loss_ce, self.total_loss_att, self.total_loss_hid = 0.0, 0.0, 0.0, 0.0
...
str2trainer = {"bert": BertTrainer, "mlm": MlmTrainer, "lm": LmTrainer,
               ...
               "seq2seq": Seq2seqTrainer, "t5": T5Trainer, "tinybert": TinyBertTrainer}
