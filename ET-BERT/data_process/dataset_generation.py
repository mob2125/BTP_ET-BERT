...
word_dir = "/home/cse/btech/cs1210557/scratch/ET-BERT/corpora/"
...
def is_retransmission(tcp, expected_seq_num):
    """
    Check if the TCP packet is a retransmission based on the expected sequence number.
    A retransmission occurs if the current sequence number is less than the expected sequence number.
    """
    #print('curr seq:', tcp.seq, 'expected seq:', expected_seq_num)
    return tcp.seq < expected_seq_num
...
def extract_dpkt(file_path):
    ...
    try:
        with open(file_path, 'rb') as f:
            ...
            for timestamp, buf in pcap_reader:
                ...
                if isinstance(ip.data, dpkt.udp.UDP):
                    udp = ip.data
                    if udp.sport == 53 or udp.dport == 53:  # MDNS
                        continue
                    if udp.sport == 1900 or udp.dport == 1900:  # SSDP
                        continue
                
                # Check for TCP retransmission (based on packet length)
                if isinstance(ip.data, dpkt.tcp.TCP):
                    tcp = ip.data
                    if last_seq_num is None:
                        last_seq_num = tcp.seq
                    else:
                        # Calculate expected sequence number
                        expected_seq_num = last_seq_num + (len(tcp.data) if last_seq_num is not None else 0)

                        # Check if the current TCP packet is a retransmission
                        #if is_retransmission(tcp, expected_seq_num):
                            # It's a retransmission, potentially containing bytes from last transmitted segment
                            # Update last_seq_num even if it's a retransmission to include new data
                            #last_seq_num = tcp.seq  # Update to current seq number
                            #continue  # Skip this packet if it's a retransmission
                        #if hasattr(tcp, 'analysis_retransmission'):
                         #   continue
                    # Update last sequence number for future checks
                    last_seq_num = tcp.seq
                ...
                if src_ip == host_ip:
                    ...
                    direction = -1
                feature_data['ip_lengths'].append(direction*len(ip))
        ...
        print(f"Error in extract_dpkt: {e}")
        return {}

def split_pcap_by_session(input_pcap, output_folder):
    # Create a dictionary to hold packets for each session
    ...
    with open(input_pcap, 'rb') as f:
        ...
        total_packets = sum(1 for _ in pcap_reader)  # get total packets for tqdm
        f.seek(0)  # reset reader to start of file

        # Loop through each packet in the pcap file with progress tracking
        for timestamp, buf in tqdm(pcap_reader, total=total_packets, desc="Processing packets"):
            try:
                eth = dpkt.ethernet.Ethernet(buf)
                if not isinstance(eth.data, dpkt.ip.IP):
                    # Ignore non-IP packets
                ...
                # Add the packet to the corresponding session
                sessions[session_key].append((timestamp, buf))
            except Exception as e:
                print(f"Error processing packet: {e}") 
    # Write each session to a separate pcap file
    for session_key, packets in tqdm(sessions.items(), desc="Writing sessions"):
        src_ip, dst_ip, src_port, dst_port, protocol = session_key
        output_filename = f"{output_folder}/{src_ip}_{src_port}_to_{dst_ip}_{dst_port}_{protocol}.pcap"
        
        with open(output_filename, 'wb') as out_f:
            pcap_writer = dpkt.pcap.Writer(out_f)
            
            # Write each packet in the session
            for timestamp, buf in packets:
                pcap_writer.writepkt(buf, ts=timestamp)

    print(f"PCAP file split into {len(sessions)} sessions.")
...
def split_cap(pcap_path, pcap_file, pcap_name, pcap_label='', dataset_level = 'flow'):
    ...
    if dataset_level == 'flow':
        #cmd = "mono /home/etbert/SplitCap.exe -r %s -s session -o " + output_path + " -p 900"
        split_pcap_by_session(pcap_file, output_path)

    elif dataset_level == 'packet':
        cmd = "mono /home/etbert/SplitCap.exe -r %s -s packets 1 -o " + output_path
        command = cmd%pcap_file
        os.system(command)
    return output_path

def cut(obj, sec, flag=False):
    if flag:
        print('sec:', sec)
...
def get_burst_feature_dpkt(label_pcap, payload_len):
    try:
        feature_result = extract_dpkt(label_pcap)
        if not feature_result:
            print("Error during extraction.")
            return -1

        ip_lengths = feature_result['ip_lengths']
        print('tcp: ', feature_result['tcp'])
        print('udp: ', feature_result['udp'])
        packet_direction = [x // abs(x) for x in ip_lengths]  # Simple direction calculation
        print(len(packet_direction))
        print(ip_lengths)
        burst_data_string = ''
        burst_txt = ''

        with open(label_pcap, 'rb') as f:
            pcap_reader = dpkt.pcap.Reader(f)
            packets = [buf for _, buf in pcap_reader]

        if len(packet_direction) == len(packets):
            for packet_index in range(len(packets)):
                packet_data = packets[packet_index]
                packet_string = binascii.hexlify(packet_data).decode()[:2*payload_len]
                #print(len(packet_data), end=" ")
                if packet_index == 0:
                    ...
                    if packet_direction[packet_index] != packet_direction[packet_index - 1]:
                        ...
                        for string_txt in cut(burst_data_string, int(length / 1)):
                            print(string_txt)
                    ...
                    if packet_index == len(packets) - 1:
                        length = len(burst_data_string)
                        tmp = cut(burst_data_string, int(length/2), True)
                        print(len(tmp))
                        for string_txt in tmp:
                            print('len:', int(length/2))
                            burst_txt += bigram_generation(string_txt, packet_len=len(string_txt))
                            burst_txt += '\n'
                        burst_txt += '\n'
            #print()
        ...
        print(f"Error in get_burst_feature_dpkt: {e}")
        return -1
...
def get_burst_feature(label_pcap, payload_len):
    ...
    try:
        ...
        print(packets)
        packet_direction = []
        feature_result = extract(label_pcap, filter='tcp or udp')
        ...
        print("error in get_burst_feature: %s"%e)
        return -1
...
def pretrain_dataset_generation(pcap_path):
    output_split_path = "/home/cse/btech/cs1210557/scratch/ET-BERT/dataset1"
    pcap_output_path = "/home/cse/btech/cs1210557/scratch/ET-BERT/dataset2/"
    
    if not os.listdir(pcap_output_path):
        print("Begin to convert pcapng to pcap.")
        print(pcap_path)
        for _parent,_dirs,files in os.walk(pcap_path):
            print(_parent,_dirs,files)
            for file in files:
                if 'pcapng' in file:
                    print(_parent + file)
    ...
    for _p,_d,files in os.walk(output_split_path + "/splitcap"):
        for file in files:
            ...
            #if num == 50:
                #break
            get_burst_feature_dpkt(_p+"/"+file, payload_len=64)
        #if num == 50:
            #break
    return 0
...
if __name__ == '__main__':
    # pretrain
    pcap_path="/home/cse/btech/cs1210557/scratch/train_data"
    # tls 13 downstream
