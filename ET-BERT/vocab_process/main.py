...
pcap_dir = "/home/etbert/verysmall"
...
vocab_dir = "/home/cse/btech/cs1210557/scratch/ET-BERT/models/"
vocab_name = "encryptd_vocab.txt"

def chunked_dataset_generator(file_path, byte_size):
...
def build_BPE():
    ...
    max_bytes = 200*1024 * 1024  # Limit chunk to 100MB
    for chunk in chunked_dataset_generator(word_dir + word_name, max_bytes):
        # Write each chunk to a temporary file
        with tempfile.NamedTemporaryFile(delete=False) as temp_file:
            temp_file.write("".join(chunk).encode('utf-8'))
            temp_file.flush()

            # Train tokenizer on the temporary file
            tokenizer.train([temp_file.name, temp_file.name], trainer=trainer)
